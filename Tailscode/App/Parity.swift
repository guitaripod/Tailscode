...
enum ParityManifest {
    ...
    static func evidence(for capability: AppCapability) -> ParityEvidence {
        ...
        case .drawLane: return .gap("Core carries the lane, the door and every word, and the composer already walks lanes — but the phone has nowhere to put the picture yet, so HomeComposerBar.offeredLanes answers three and the lane is never drawn; the forge's own full-screen surface is the shape this owes")
