...
enum ParityManifest {
    ...
    static func evidence(for capability: AppCapability) -> ParityEvidence {
        ...
        case .unreadTracking: return .partial("SessionSeenStore", missing: "a manual read/unread toggle; the badge sets and clears itself but cannot be flipped by hand")
        case .savedChats: return .implemented("SavedChatsViewController")
        case .archivedChats: return .gap("no ArchivedChatStore wiring; a chat can only be saved or deleted, never set aside")
        ...
        case .rowContextActions: return .partial("sessionMenu", missing: "archive, fork and read/unread verbs in the row menu")
        ...
        case .transcriptFind: return .partial("presentJumpSheet", missing: "text search with a match count; the jump sheet only reaches user prompts")
        ...
        case .statusBand: return .partial("renderNavStatus", missing: "the token estimate; the nav status does not read the shared StatusFacts")
        ...
        case .keyboardShortcuts: return .gap("no UIKeyCommand bridge to ShortcutRegistry; a hardware-keyboard iPad gets no app shortcuts")
        case .shortcutCheatsheet: return .gap("nothing to derive a cheatsheet from until the registry is wired")
