...
Latest releases: **iPhone 1.45** · **macOS 1.45** · **Linux 1.48**. The apps are free; a one-time **$14.99 Pro** non-consumable unlocks unlimited servers and concurrent Live Activities on both iPhone and Mac. Universal since 1.26 — iPad gets its own readable-column layout and multiple scenes, not a stretched phone screen.
...
**Arch** — [tailscode](https://aur.archlinux.org/packages/tailscode) builds the current release from the `v1.48` tag; [tailscode-git](https://aur.archlinux.org/packages/tailscode-git) tracks master:
...
```bash
tar xf tailscode-1.48-linux-x86_64.tar.gz -C ~/.local --strip-components=2
```
