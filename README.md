...
Latest releases: **iPhone 1.47** · **macOS 1.45** · **Linux 1.52**. Free is the whole core with one server; **Pro** is a one-time $14.99 purchase for unlimited servers, concurrent Live Activities and the delegate board.
...
**Arch** — [tailscode](https://aur.archlinux.org/packages/tailscode) builds the `v1.52` tag; [tailscode-git](https://aur.archlinux.org/packages/tailscode-git) tracks master.
...
```bash
tar xf tailscode-1.52-linux-x86_64.tar.gz -C ~/.local --strip-components=2
```
