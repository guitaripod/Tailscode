...
enum ParityManifest {
    ...
    static func evidence(for capability: AppCapability) -> ParityEvidence {
        ...
        case .slashCompletion: return .partial("CompletionPopover", missing: "no argument stage or hints")
        case .slashDispatch: return .implemented("SlashDispatch.decide")
        case .commandCatalog: return .gap("No browsable catalog; the completion popover is the only way to meet a command")
        ...
        case .chatDragToPane: return .gap("a chat row cannot be picked up and a pane takes no drop")
        case .clickToActivate: return .implemented("pressLanded")
        case .uiScale: return .implemented("UIScale")
        case .themePicker: return .gap("Takes its colours from the system accent and appearance; the named themes are Linux-only so far.")
