...
enum MacAppInstall {
    ...
    static func reading(at checkedAt: Date = Date()) -> UpdateReading {
        ...
        return UpdateReadings.app(
            ...
            command: buildCommand(at: state.path), projectURL: projectURL, checkedAt: checkedAt,
            subtitle: subtitle)
    }
    ...
    /// successful fetch left behind, and a surface that printed "up to date" off a week-old remote
    /// ref would be claiming a check nobody performed.
    ...
    static func checkoutReading() -> CheckoutReading {
        guard let path = sourceDirectory(), let porcelain = git(["status", "--porcelain"], in: path)
    ...
    /// Whether the machine holds what a rebuild needs, answered honestly rather than forced to no.
    /// It is not the same question as whether *this app* can take an update — see `obstacle` — but
    /// a machine missing Xcode or xcodegen cannot even run the command it is handed, and Core says
    /// so in its own words once `canBuild` is false.
    ...
    /// Core prints the obstacle inside the verdict, and a row that stated it twice would read as
    ...
    ///
    /// `CheckoutState.blocker` speaks a fixed vocabulary and has no case for the real one: the tree
    /// can be clean, the toolchain present, and the build would still land on the very bundle this
    /// process is executing out of. It is said only where a press exists — a checkout with nothing
    /// to pull is up to date, and answering "cannot update itself" to a machine with nothing to
    /// install would trade a useful fact for a true irrelevance. Whatever the checkout itself
    /// objects to rides along, because the command handed over would hit that first.
    private static func obstacle(for checkout: CheckoutState) -> String? {
        ...
        let mine = Localized.text(
            "A rebuild takes xcodegen and a multi-minute xcodebuild, and what it produces is the "
                + "bundle this process is running from — so the command is handed over rather than "
                + "pressed here.")
        guard let blocker = checkout.blocker else { return mine }
        return "\(mine) \(blocker)"
    }

    /// The line that actually replaces this app, rather than Core's `git pull` fallback — which
    /// fetches the source and leaves the running bundle exactly as it was.
    ...
    /// The checkout this build came from, found by walking up from the bundle: an app run out of
    /// `build-mac/Build/Products/Debug` is five directories below the tree that made it. A
    ...
    /// output and be parsed as a version. The search path is widened by hand instead, because an
    /// app launched from Finder inherits `/usr/bin:/bin` and would never find a Homebrew
    /// xcodegen. Git is told to ask for nothing — a background check that stopped on a credential
    /// prompt would hang until the process died.
