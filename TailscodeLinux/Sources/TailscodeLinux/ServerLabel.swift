...
enum ServerLabel {
    static func agent(_ backend: AgentType) -> String {
        backend == .openCode ? "opencode" : "Claude Code"
    }

    static func display(name: String, backend: AgentType) -> String {
        let agent = agent(backend)
        guard name.range(of: agent, options: .caseInsensitive) == nil else { return name }
        return "\(name) · \(agent)"
    }

    static func display(_ profile: ConnectionProfile) -> String {
        display(name: profile.name, backend: profile.backend)
    }

    static func address(_ profile: ConnectionProfile) -> String {
