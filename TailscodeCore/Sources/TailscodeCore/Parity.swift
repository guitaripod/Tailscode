...
public enum CapabilityRegistry {
    public static let all: [CapabilityDefinition] = [
        ...
        CapabilityDefinition(
            id: .splitTabs, area: "chat list", title: "A split is one row, and it opens",
            spec:
                "A window holding several conversations is remembered as one thing and offered back by the chat list. SplitTabStore records the arrangement every time a window persists one — the tree, the panes' chats in reading order, and any page or stream open beside them — keyed on the *set of conversations* rather than the tree, so splitting the same two chats again is the same row wearing a new shape rather than a second row saying the same names; the list is capped and device-local, because a tab is a shortcut back to a habit and not a second chat list. SplitTabGrouping folds the tabs into the sections every client already builds: the row takes the place of the first of its members, which puts it in the highest section any of its chats earns, and the state it wears is the loudest pane's — a split with a question waiting in one pane is a split that needs you. It is only ever drawn whole: a listing missing one member — filtered out, archived, deleted, on a machine that stopped answering — cannot honestly be offered, so its surviving chats are drawn as the plain rows they always were. Pressing it puts the whole arrangement back through the same binding machinery a launch restore uses, so a member whose server cannot answer keeps its pane instead of failing the gesture; every member still has a line of its own that opens that chat alone, marking the row holds every chat in it, and the arrangement can be let go of by hand without touching the chats. A platform with no tiling answers notApplicable — there is no window arrangement for a row to stand for."),
        CapabilityDefinition(
