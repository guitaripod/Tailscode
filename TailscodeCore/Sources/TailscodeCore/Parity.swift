...
public enum CapabilityRegistry {
    public static let all: [CapabilityDefinition] = [
        ...
        CapabilityDefinition(
            ...
            spec:
                "A Workflow call renders as the run it started, not the receipt it returned: the workflow's name and description, the phase plan its script declares, live agent rows (what each is doing, for how long), a progress reading over the fan-out in hand, elapsed, and — when the run's task reports back — the answer folded into the same card. Phase attribution is only claimed once the finished run records it; a live card shows the plan and the agents, never a guessed position between them. A run ends because the harness said so, never because a clock ran out: the launching call answers in milliseconds and the work runs for minutes, so the only ending is the report the harness posts afterwards, carried to the client as ToolCall.background rather than paraphrased into prose the card would have to parse back. WorkflowRun.State turns that into finished, stopped or failed — a run that was killed is over without being blamed for a fault — and the card wears exactly one mark for it, run.activityIcon, so the header cannot disagree with the agent rows under it. Every ending holds perfectly still, because a record that keeps moving reads as work that never ended — the agent rows included: an agent's sidecar goes on reporting itself active for half an hour after the run around it is over, so ActivityIcon.workflowAgent(_:in:) reads the run and settles every row the moment the run settles. The phase rail answers to WorkflowRun.phaseStanding rather than to isLive: only a reported completion fills it, because nothing records which phase a run was in when it was stopped or when it broke, and a killed four-phase script drawn as four finished phases claims work the transcript never saw. Nothing about a run is measured against the reader's clock — an ending is a fact about when the work stopped, so a run whose report carried no time reports no length at all rather than one that grows every time the conversation is reopened. The rows are timed the same way, because an agent that never reported finishing has no ending of its own either: WorkflowAgent.elapsed(at:in:) reads the run too, and an agent inside a run that is over stopped when the run stopped — at the latest — so a card reopened a week later shows the errand it ran, not seven days of it."),
        CapabilityDefinition(
        ...
        CapabilityDefinition(
            ...
            spec:
                "Sending scrolls the conversation so the prompt just written rests at the top of the window (FreshCanvas.headroom under the edge) and everything under it is empty, so the answer streams onto a blank canvas rather than into the last inch above the composer; the rest of the conversation is one scroll up. The room is made, not found: the client pads the end of the transcript by FreshCanvas.padding — the viewport less the prompt and whatever already sits under it — and the padding shrinks as the answer grows into it, until the end of the conversation meets the end of the window and ordinary follow-the-bottom takes over. The room stays when the turn ends — a short answer under its question is the page that was asked for, never snapped back to the bottom — and goes only on the next send or on leaving the chat. The move is animated on the display's own clock, one motion from where the reader was to where the prompt lands, never a jump followed by a second correction; a prompt sent from far up the transcript rides the same motion. Reduced motion lands at once. When the server's row replaces the pending one the prompt does not move, because the swap happens at the offset the reader is already at. A queued message, which has not gone, does not rise — only a send does. The block that rises is the whole send: a picture clipped to the words is docked above them, so the first row of the send is what rests under the top edge, never the words with the picture hidden a row above. The room is measured against what the reader can see, not what is laid out: the live row is laid out in full ahead of its reveal, so the padding shrinks and the canvas lets go by the written height (FreshCanvas.below with the live row's unrevealed part taken off), and once it has gone a reader at the bottom is kept on the last written line (FreshCanvas.followOffset) on every frame the reveal advances — the page is pushed up by the writing, never by the layout that ran ahead of it."),
        CapabilityDefinition(
        ...
        CapabilityDefinition(
            id: .unifiedModelChooser, area: "composer", title: "One selector, machine first",
            spec:
                "Every model on every server is chosen from one searchable surface built on the shared ModelChooser — never a flat menu of the raw catalog. The first choice is the machine: every connected server is a tab (ModelChooser.machines, ⌃1–9, the one the chooser opened from first and preselected), each tab wearing its model count, how many are local, and whether it is answering, and the list under a tab is that one machine's catalog and nothing else's — the old merged alphabet, where the same model appeared once per server with the server as a badge on each, is gone, and so are the standing filters it needed (this server / local). A tab that is not this chat's server says once, above its list, that a pick there starts a new chat on that machine (ModelMachine.consequence) instead of saying it on every row. Under the tab: Yours — the model this chat runs, named rather than implied by a tick down the page, then starred, then recent, deduplicated into one short section with the server's own default closing it, where three headings used to stand; then the models running on the machine's own hardware as a section of their own (they never sit in a family beside a hosted namesake); then the families. A row is a name over what settles which model it is — the door a pick would take with the count of the others (Anthropic +1) and the id when it says more than the name — and wears only what would stop a send or what the whole catalog does not share: the wall in front of it, and capabilities in the quieter register with any capability every row shares dropped by ModelFactPolicy. Effort levels and provider counts are no longer pills; effort is chosen after the pick. A search asks the shown machine and answers from the others too, under one heading that names the machine on each row, so the tab never hides a model somebody typed the name of; an empty answer names the machine as well as the word. Past ModelChooser.foldFrom hosted models the families fold: the list arrives open only where the model in use lives, each heading is a press (⌃→/⌃←, ⌃⇧ for all) that says how many it holds, and one control at the top opens everything or shuts it again. A wall is the account's rather than the machine's. The door a collapsed row picks is deliberate: the door the chat is already on wins, then the model's own house over a reseller fronting it. A star (ModelFavoritesStore) puts a model in Yours on the machine that runs it and follows it through every section; the quick menu shows ModelChooser.shortlist over the whole fleet and hands the rest to this surface, so the two are one list at two lengths. Under the machine's tab the doors are a filter (ModelChooser.doors/setDoor, ⌥1–9, ⌥0 for all): one chip per provider the machine reaches its models through — a subscription, a key, a gateway — wearing how many it runs, drawn only past one door, so a person who pays for Ollama Cloud and for OpenCode Go can read what each will run as one list rather than one folded row at a time; a row under a door names that door alone and a pick goes through it, the summary says which door is showing, an empty answer names the door, and a tab change lets the door go because a door is a fact about the shown machine. A heading's chevron and a row's always point the way the list is actually folded — a supplementary view is not an item and a diffable snapshot never reconfigures it, so every client rewrites what is on screen after a fold."),
        CapabilityDefinition(
        ...
        CapabilityDefinition(
            ...
            spec:
                "A person reading an answer asks three things about it: was that slow, was that expensive, what wrote it. ResponseStats answers them under the turn itself as one quiet strip — output speed, time to first token, elapsed, tokens written, tokens read, estimated cost, and the model with its effort — off by default (ResponseStatsSetting) because a transcript is for reading and a rail of figures under every answer is a tax on the reading it exists to inform. It is deterministic and never measured here: every figure is the server's own arithmetic or a division of two of the server's own numbers, so two devices watching one conversation report the same answer identically and nothing drifts with the network or this process's own scheduling. The rate divides MessageUsage.written and never a total — a turn that read a hundred thousand cached tokens and wrote two hundred did not run fast — and needs enough clock under it to mean anything. Where the server stamped the first output token (MessagePart.startedAt), the wait is split: time to first token is its own figure and the rate is measured over the generation that followed, so a long prompt being read is never billed as the model's slowness; a backend that stamps nothing gets one honest rate over the whole wait and no first-token figure invented for it. The speed's tooltip may add what the model has averaged on this device (ModelSpeedLedger, decayed so provider drift ages out), which is memory, not measurement, and lives only in the detail. What cannot be derived is left out rather than guessed: a turn whose server said nothing about tokens shows its clock and no rate, a turn that said nothing at all draws no strip, and a turn still being written or one that failed draws nothing, because a rate over a partial answer moves under the reader's eye and everything settled in this app holds perfectly still. Money follows the spend doctrine — API-equivalent value, marked an estimate, never a bill. Each client decides only the layout: the same facts, the same order, the platform's symbol or the text glyph, the detail sentence as a tooltip and the whole strip spelled out for a screen reader."),
        CapabilityDefinition(
        ...
        CapabilityDefinition(
            ...
            spec:
                "The only moment the app asks for a review is the moment value just landed, and the policy that decides whether to ask is Core's, not a client's: ReviewPromptPolicy counts successful turns and answers due on three signals, one ask per cooldown window so a review is never nagged for, and with no install-age gate — a week of calendar time says nothing a completed turn does not. What counts as a successful turn is the ordinary reading: the turn finished, produced content, and failed at nothing; five of them make an ordinary turn's end due. A trophy earned waives the turn count, because that moment is the deepest signal the person already has. Strongest of all is coming back: the person left the app while a turn was running and returned to a finished answer — the product's whole pitch, kept — and that is due after just two successful turns, asked a few seconds after the app is active again so it lands on someone reading the result. The ask itself is the platform's own store-review call (SKStoreReviewController) — nothing renders a custom prompt, so a dismissal the system never reports back is not something the policy pretends to know; the system's own annual cap is the last word on how often anyone is asked. A platform without a store to review answers notApplicable, and the policy is never told to ask there. The ask is debounced behind the turn's end so it lands on the person reading the answer, not on the frame the answer arrived."),
        CapabilityDefinition(
        ...
        CapabilityDefinition(
            id: .supporterInvitation, area: "settings", title: "One invitation to fund the work, earned by use",
            spec:
                "Most people run one machine and never meet the server gate, so they never learn the unlock exists; the answer is one dismissible invitation, earned by use rather than by time. SupporterInvitation in Core counts turns that finished well and is due at ten, never while Pro is held, and never again once set aside or bought (settle). The copy is Core's — the title, ProOffer.pitch plus the one-purchase, iPhone-and-Mac, unlimited-servers line, and a primary action carrying the store's price once it has answered — and it keeps ProOffer's stance whole: no wall, no trial, nothing on launch, and a second ask never happens. A client with nothing to sell renders nothing."),
        CapabilityDefinition(
            id: .videoForge, area: "splits", title: "A video is asked for and watched being made",
            spec:
                "A person describes a video on any client and watches it be made on the machine with the card. Everything but the drawing is Core's. Where the renderer lives is a ForgeEndpoint — a host and ComfyUI's own 8188 read through the same HostAddress parser the agent connection uses, kept in ForgeStore, and probed with PortReachability rather than an HTTP request so \"nothing is listening\" is told apart from \"that machine is asleep\" and \"that name does not resolve\" (ForgeEndpoint.sentence names each). What is rendered is a ForgeRecipe — prompt, negative, a frame size that is a multiple of 32, seconds, frame rate, seed and which of the two LTX-2.5 transformers, each model bringing its own sigma schedule — and ForgeGraph turns it into the twenty-eight-node two-pass graph verified on the box, encoding the invariants as code rather than as comments: length is seconds times rate plus one, the first pass samples at half the asked-for size and the latent upsampler doubles it back, and ForgeGraph.problems refuses to post a graph whose links do not resolve. ForgeClient runs it: one client id shared by the POST and the websocket or the frames land on somebody else's socket, a 45-second budget on the first request because the box is socket-activated and takes about twelve seconds to answer cold, every frame type parsed by ForgeEvent including the null-node executing frame that is how a finished render announces itself, /interrupt to cancel, and the file resolved out of history to a /view URL — with a salvage loop so a phone that locked its screen mid-render still gets its clip. Progress is the node census from progress_state (finished over total), never the sampler's step counter, which restarts between the two passes and would fill, empty and fill again. ForgeJob is the state machine (drafting, submitting, queued, running, done, failed, cancelled) and owns every word on screen — title, subtitle, detail, badge, elapsed, which pass is working — and ForgeBoard lays it out as four sections every client draws identically: the renderer and whether it answered, the render in hand with its bar, the settings as walkable rows, and the clips already made. A failure is never silent: ForgeFailure carries a sentence naming the machine and, where it can, the fix, and ForgeClient.reason normalises anything foreign so no client ever prints an error object. What each client owes is the drawing — rows, a text field, a video player for ForgeAsset.url(on:) — and nothing else. How the surface is reached and how the machine behind it is chosen are capabilities of their own (forgeEntry, forgeSetup), because a renderer nobody can find and a box nobody can open are the two ways this feature is present and unusable."),
        CapabilityDefinition(
            id: .forgeEntry, area: "app", title: "Video is a first-class action, opened over the work",
            spec:
                "A render is a task you start, watch and collect — not a place you work — so it is reached like an action and presented like one. The control is first-class and visible with the app’s other primary ones rather than buried in a settings menu or a menu bar: one title, one SF Symbol and one text glyph everywhere (ForgeEntryPoint), a tooltip that promises something different before a renderer exists than after, and a live mark while a render is out (ForgeEntryPoint.activity) so somebody who closed the surface can still see the other machine working. Opening it opens a modal over the window — a sheet on the Mac, a modal transient-for window on Linux, a presented screen on the phone — never a pane in the tiling grid: a surface nobody types into for four minutes does not earn half a transcript’s width, and it left a pane behind afterwards. The transcript is untouched behind it and comes back whole when the modal closes. Every desktop opens it at the same size (ForgeSurface.preferredWidth/Height, with a floor) and closes it the same way (ForgeSurface.dismissTitle, and Escape where the platform means Escape). The one thing dismissal may never do is stop the render: the job and its socket live above the surface the way ForgeRunner already holds them on iOS, so closing is closing a window rather than cancelling minutes of somebody else’s card — and while a render is out the surface says exactly that (ForgeSurface.dismissNote). Reopening finds the same render where it was. A keyboard route stays alongside the control rather than replacing it."),
        CapabilityDefinition(
            id: .forgeSetup, area: "servers", title: "The renderer is set up the way a server is",
            spec:
                "Pointing at the renderer is held to the standard adding a server is held to, and every word and rule of it is Core’s (ForgeSetup). The surface states what this machine’s own tailnet address is before asking for anybody else’s, because a probe from a machine with no tailnet fails identically however right the address is. The address is not typed blind where it can be found: ForgeSweep asks every machine the agent scan would ask (TailnetScanner.scannableDevices, so phones and peers nobody has seen in ten minutes are skipped) whether anything answers on ComfyUI’s 8188, using PortReachability rather than an HTTP probe — TailnetScanner itself cannot serve here, since its probe discards everything that is not an agent, which is every ComfyUI there has ever been. Each machine is asked by MagicDNS name first and by one address behind it, and only a name that failed to resolve is worth trying the second address on (ForgeSweep.step). The wait is TailnetRadar’s dial, unchanged: every machine takes its stable place before a probe lands, answers light it, and what never answered leaves the dial rather than reading as a maybe (ForgeSweepReading). A machine a scan found is adopted already checked. Anything typed is read by the same HostAddress parser the agent connection uses, so a bare name, a MagicDNS name, host:port, a URL or a whole line pasted out of ComfyUI’s terminal all work, the supplied port is said out loud (ForgeSetup.hint), and the three refusals are ForgeEndpoint.complaint’s. What is typed is checked before it is saved, and a failure explains itself in a sentence naming the fix rather than a raw error: ConnectDiagnosis.forge maps the three verdicts onto the same value the server flow uses — off the tailnet and a name that will not resolve are literally the server flow’s own sentences, while a port refusing on a machine that is up is the renderer’s own and hands over the line that starts ComfyUI bound where the tailnet can see it (ConnectDiagnosis.Fix.copyCommand). One rule is the renderer’s rather than the server flow’s: a machine that said nothing may still be the right machine, because ComfyUI is socket-activated and stops itself when idle — so silence and a refusal may be saved anyway with the reason stated (ForgeSetup.secondary), while a name this device cannot resolve may not, since that files an address that can never work. The answer is remembered: ForgeStore keeps every renderer this device has pointed at, most recent first, named the way the tailnet names it, so the second time is a tap; a name learned later replaces a blank one, forgetting one stops sending work to it, and the machine in force wears a word (ForgeRenderer.badge). A client owes a text field, a list, a radar and buttons. It decides nothing and writes no sentence."),
        CapabilityDefinition(
        ...
        CapabilityDefinition(
            id: .videoLane, area: "composer", title: "The prompt box has a video lane",
            spec:
                "A video is asked for in words, and the words are typed where words are already typed: the front door's prompt box carries video as its third lane beside chat and the quick ask, so describing a clip costs no navigation. The lanes are Core's (QuickAskLane: order, word, symbol, placeholder, spoken, sendLabel), and the lane in force is stated rather than implied — the switch wears the lane's own word and symbol, the accent whenever the box is anywhere but chat, the empty box says what this lane is for, and the send control changes face and name (Render, never Send), because the video lane spends minutes of another machine's card and a send nobody meant must be impossible to miss. The flip is a tap on the switch — one walk through the three lanes — and, where the platform has a finger, a horizontal swipe on the box itself: the bar follows the drag with weight (ComposerLaneSwipe.offset), commits only past a deliberate distance (ComposerLaneSwipe.threshold), lands with the selection haptic and a spring, and a drag that stays short springs back having changed nothing. A desktop has no swipe and does not fake one: the same three lanes sit on its composer's own strip as buttons, and because each lane on a desk is a surface of its own, a press is a door — chat is the pane it was pressed in and wears the accent, ask opens the question window the summon chord opens, video opens the forge — while the words in the box stay the conversation's. The lane's tools appear only while the lane does: the destination chip names the renderer the way the chat lane names its project — or says none is set up and opens the setup — and under the box the walkable settings ride as chips (QuickAskLane.videoChips), each cycled with the same tap the board answers, plus the road into the full surface. The words in the box are the forge's own draft (the recipe prompt), so the lane and the video surface can never hold two different prompts — typed here is typed there, and both survive a restart with the recipe. Sending is ForgeRunner's begin: a configured renderer starts the render and opens the video surface over the work so the first seconds of waiting are watched rather than trusted; a missing renderer opens the same surface leading with its setup instead of failing the send. The lane resets to chat when the front door is next stood up — a composer that quietly stays in video mode is how a prompt meant for the agent burns a render — while the words, like every draft, survive the flip."),
        CapabilityDefinition(
